...
export const GridBody: React.FC<GridBodyProps> = ({
  ...
  const handleMouseMove = useCallback(
    ...
    [offsetLeft, scrollX, columnWidth, viewMode, dates, rowHeight]
  );
  ...
  const invalidBarClick = useCallback(() => {
  ...
  const { invalidColumn, gridRows, rowLines, y } = useMemo(() => {
    let y = 0;
    ...
    return { invalidColumn, gridRows, rowLines, y };
  }, [
    tasks,
    svgWidth,
    rowHeight,
  ...
  const { ticks, today, invalidBar } = useMemo(() => {
    const now = new Date();
    let tickX = 0;
...
export const MemoGridBody = memo(GridBody);
